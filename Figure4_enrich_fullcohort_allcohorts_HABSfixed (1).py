...
"""
Figure4_enrich_fullcohort_allcohorts_HABSfixed.py
...
OUTDIR = BASE_VALIDATION_DIR / "Figure4_enrichment_full_cohort"
...
def compute_hc_graph_metrics(
    ...
    return {
        "Hc_clustering_coeff": float(hc_clustering) if np.isfinite(hc_clustering) else np.nan,
        "Hc_path_length": float(hc_path) if np.isfinite(hc_path) else np.nan,
        "graph_status": "ok",
    }
