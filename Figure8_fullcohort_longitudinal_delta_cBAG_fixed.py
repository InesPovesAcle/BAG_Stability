...
# -*- coding: utf-8 -*-
"""
Figure8_fullcohort_longitudinal_delta_cBAG_fixed.py
===================================================

Fixed Figure 8 script for longitudinal biological validation.
...
------------
From terminal:
    python Figure8_fullcohort_longitudinal_delta_cBAG_fixed.py --cohorts ADNI,HABS --feature-set full

If you want HABS only:
    python Figure8_fullcohort_longitudinal_delta_cBAG_fixed.py --cohorts HABS --feature-set full

If you want imaging_only:
    python Figure8_fullcohort_longitudinal_delta_cBAG_fixed.py --cohorts ADNI,HABS --feature-set imaging_only
...
DEFAULT_OUTDIR_NAME = "Figure8_fullcohort_longitudinal_delta_cBAG_fixed"
...
def select_main_endpoints(stats: pd.DataFrame, max_endpoints: int) -> List[str]:
    ...
    available = set(pooled["endpoint"])
    for e in preferred:
        if e in available and e not in endpoints:
            endpoints.append(e)
...
def make_figure8(delta_df: pd.DataFrame, stats: pd.DataFrame, main_endpoints: Sequence[str],
    ...
    # If preferred panels are unavailable, replace with selected main endpoints.
    selected_available = [e for e in main_endpoints if f"delta__{e}" in delta_df.columns]
    for i, e in enumerate(endpoint_for_panel, start=1):
        if e not in selected_available:
            replacement = selected_available[min(i - 1, len(selected_available) - 1)] if selected_available else e
            e = replacement
    ...
    return save_all(fig, outdir / "Figure8_fullcohort_longitudinal_delta_cBAG_fixed", formats, dpi)
...
def make_supp_figure8(delta_df: pd.DataFrame, stats: pd.DataFrame, endpoints: Sequence[str],
    ...
    fig.suptitle(title, fontsize=15, y=0.995)
    fig.tight_layout(rect=[0, 0, 1, 0.97])
    return save_all(fig, outdir / "Supplementary_Figure8_fullcohort_longitudinal_delta_cBAG_fixed", formats, dpi)
...
def main() -> None:
    ...
    manifest = pd.DataFrame([
        {
            "figure": "Figure 8",
            "description": "Longitudinal within-person full-cohort delta-cBAG biological validation",
            "feature_set": args.feature_set,
