...
def add_harmonized_columns(df: pd.DataFrame, cohort: str) -> tuple[pd.DataFrame, str]:
    ...
    if cohort == "ADNI":
        ...
        subj = first_existing(out, ["Subject", "subject_id"])
        sess = first_existing(out, ["runno", "connectome_key", "connectome_full_key", "graph_id"])
    elif cohort == "AD_DECODE":
...
def make_feature_set_consistency(diag_all: pd.DataFrame) -> pd.DataFrame:
    out = diag_all.copy()
    return out.sort_values(["cohort", "feature_set"])
...
def save_formatted_excel(table1_values: pd.DataFrame, output_path: Path) -> None:
    ...
    ws.cell(current_row, 1).value = (
        ...
        "AD_DECODE Risk 0 and Risk 1 were combined as NORMCOG/control. APOE ε4 strict carriage excludes APOE2/4."
    )
...
def main() -> None:
    ...
    readme.write_text(
        ...
        "ADNI diagnosis source:\n"
        "- File: `$WORK/ines/data/harmonization/ADNI/metadata/ADNI_Metadata/MetaData/All_Subjects_DXSUM_26Mar2026.csv`\n"
        ...
        "- ADRC: `NORMCOG`, `IMPNOMCI`, `DEMENTED`.\n"
        "- HABS: `CDX_Cog` where 0=CN, 1=MCI, 2=AD/Dementia.\n"
