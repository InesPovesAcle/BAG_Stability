...
Input priority per cohort/model:
  ...
  Controls-only sensitivity Table S2:
    1) <prefix>_<feature_set>_metadata_all_with_predictions.csv
    2) validation_figures/subject_level_validation_input.csv
    3) <prefix>_<feature_set>_full_cohort_predictions.csv as last fallback

Controls-only uses metadata-enriched files first because full_cohort_predictions
often contains only graph_id, age, and predictions, without diagnosis labels.
...
def candidate_input_paths(cohort: str, feature_set: str, subset: str = "all_subjects") -> list[tuple[str, Path]]:
    ...
    if subset == "controls_only":
        ...
        paths.append(("full_cohort_predictions_last_resort", full_cohort_predictions_path(cohort, feature_set)))
        return paths
...
def metrics_for_one(cohort: str, feature_set: str, subset: str = "all_subjects") -> dict:
    ...
    if subset == "controls_only":
        # First try the selected input table. For controls-only, this should
        # usually be metadata_all_with_predictions.
        ...
        # from metadata_all_with_predictions.
        if control_mask.sum() == 0:
        ...
        # If the prediction file lacks cognitive-status labels, use the same
        # harmonized labels already used by Figure 5/AUC cognitive-status analyses.
        if control_mask.sum() == 0:
        ...
        if control_mask.sum() == 0:
            ...
            # Save compact status-column diagnostics in diagnostic_all_attempts.
...
def main() -> None:
    ...
    readme.write_text(
        ...
        "Controls-only sensitivity Table S2: metadata-enriched predictions first, then validation input, then full-cohort predictions as last resort.\n\n"
