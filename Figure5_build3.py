...
def select_domain_associations_complete(assoc: pd.DataFrame) -> pd.DataFrame:
    ...
    for cohort in COHORTS:
        for category, families in MAIN_COLUMNS.items():
            ...
            # CRITICAL FIX:
            # Convert the selected pandas Series to a plain dict before appending.
            # Otherwise selected becomes a mixed list of dicts + Series. In some pandas
            # versions that crashes with:
