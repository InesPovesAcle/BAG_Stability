...
"""
Figure4_enrich_fullcohort_allcohorts_HABSfixed_WITH_GLOBAL_GRAPH.py
...
OUTDIR = BASE_VALIDATION_DIR / "Figure4_enrichment_full_cohort_WITH_GLOBAL_GRAPH"
...
def compute_hc_graph_metrics(
    ...
    """
    Compute hippocampal and whole-connectome graph metrics from one adjacency matrix.

    Previous version only returned:
        Hc_clustering_coeff
        Hc_path_length
    ...
    Therefore HABS can contribute longitudinal graph endpoints to Figure 8.
    """
    mat = pd.read_csv(connectome_path, header=None).to_numpy(dtype=float)
    ...
    for u, v, d in G.edges(data=True):
        w = mat[u, v]
        d["weight"] = float(w)
        d["distance"] = float(1.0 / w) if w > 0 else float("inf")

    valid_nodes = [n for n in hip_nodes_zero_based if n in G.nodes]

    # Hippocampal clustering/path length
    if valid_nodes:
    ...
    # Whole-connectome weighted clustering
    try:
        all_clustering = nx.clustering(G, weight="weight")
        total_graph_clustering = np.nanmean(list(all_clustering.values()))
    except Exception:
        total_graph_clustering = np.nan

    # Whole-connectome characteristic path length.
    # Use finite shortest paths across connected components.
    try:
        total_path_values = []
        for node in G.nodes:
            lengths = nx.single_source_dijkstra_path_length(G, node, weight="distance")
            total_path_values.extend([
                v for target, v in lengths.items()
                if target != node and np.isfinite(v)
            ])
        total_graph_path = np.nanmean(total_path_values) if total_path_values else np.nan
    except Exception:
        total_graph_path = np.nan

    # Efficiency metrics.
    # NetworkX global_efficiency/local_efficiency are unweighted, so for a weighted
    # connectome we compute weighted efficiency using distance = 1 / weight.
    try:
        n = G.number_of_nodes()
        inv_dist_values = []
        for node in G.nodes:
            ...
            for target, dist in lengths.items():
                if target != node and np.isfinite(dist) and dist > 0:
                    inv_dist_values.append(1.0 / dist)
        global_eff = (
            float(np.nansum(inv_dist_values) / (n * (n - 1)))
            if n > 1 and len(inv_dist_values) else np.nan
        )
    except Exception:
        global_eff = np.nan
    ...
    try:
        ...
        for node in G.nodes:
            ...
            k = sub.number_of_nodes()
            inv_dist_values = []

            for source in sub.nodes:
                lengths = nx.single_source_dijkstra_path_length(sub, source, weight="distance")
                for target, dist in lengths.items():
                    if target != source and np.isfinite(dist) and dist > 0:
                        inv_dist_values.append(1.0 / dist)

            if k > 1 and inv_dist_values:
                local_eff_values.append(np.nansum(inv_dist_values) / (k * (k - 1)))

        local_eff = np.nanmean(local_eff_values) if local_eff_values else np.nan
    except Exception:
        local_eff = np.nan
    ...
    return {
        ...
        "Total_graph_clustering_coeff": finite_or_nan(total_graph_clustering),
        "Total_graph_path_length": finite_or_nan(total_graph_path),
