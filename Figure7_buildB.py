...
def compute_endpoint_correlations(df: pd.DataFrame, cohorts: Sequence[str], min_n: int = 8) -> pd.DataFrame:
    ...
    for endpoint, spec in ENDPOINTS.items():
        ...
        for cohort in cohorts:
            ...
            if sub.empty:
                rows.append({"endpoint": endpoint, "domain": domain, "cohort": cohort, "column": None, "n": 0, "r": np.nan, "p": np.nan, "ci_low": np.nan, "ci_high": np.nan, "status": "missing_cohort"})
                continue
            ...
            if col is None:
                rows.append({"endpoint": endpoint, "domain": domain, "cohort": cohort, "column": None, "n": 0, "r": np.nan, "p": np.nan, "ci_low": np.nan, "ci_high": np.nan, "status": "missing_endpoint"})
                continue
            ...
            if n < min_n or x[ok].nunique() < 2 or y[ok].nunique() < 2:
                rows.append({"endpoint": endpoint, "domain": domain, "cohort": cohort, "column": col, "n": n, "r": np.nan, "p": np.nan, "ci_low": np.nan, "ci_high": np.nan, "status": "insufficient_n_or_variance"})
                continue
...
def plot_forest_panel(
    ...
    for c_idx, cohort in enumerate(cohorts):
        ...
        for e_idx, endpoint in enumerate(endpoints):
            ...
            if pd.notna(lo) and pd.notna(hi):
                ax.plot([float(lo), float(hi)], [y, y], linewidth=0.8, alpha=0.55)
            ax.scatter(
                [r],
                [y],
    ...
    for e_idx, endpoint in enumerate(endpoints):
        ...
        if pd.notna(meta_r):
            ax.plot([float(meta_r), float(meta_r)], [e_idx - 0.38, e_idx + 0.38], linewidth=2.2)
    ...
    ax.invert_yaxis()
    if show_legend:
        leg1 = ax.legend(fontsize=7, frameon=False, loc="lower right", title="Cohort")
        if leg1 is not None:
    ...
    if not valid_stats.empty and n_max > n_min:
        ...
        ax.legend(handles, labels, fontsize=6, frameon=False, loc="upper right", title="Dot size")
        if show_legend:
            ax.add_artist(leg1)
...
def make_figure7(
    ...
    plot_forest_panel(ax_c, endpoint_stats, replication_summary, cohorts, max_endpoints=max_endpoints, endpoint_order=endpoint_order, show_legend=True)
    panel_label(ax_c, "C")
