...
def get_graph_identifier(data, idx):
    # Prefer scan/session-level IDs first. This is essential for longitudinal
    # ADNI/HABS outputs because subject-level IDs such as match_id collapse
    # visits and prevent y0/y2 or y0/y4 pairing in downstream figures.
    candidate_keys = [
        "connectome_key", "runno", "graph_id", "Subject_ID", "subject_match",
        "regional_id", "match_id", "subject_id", "PTID", "ptid"
    ]
    for key in candidate_keys:
        if hasattr(data, key):
...
def find_best_metadata_merge_key(metadata_df, graph_ids):
    # Prefer scan/session-level keys before subject-level keys. If graph IDs
    # are H4369_y0/H4369_y2, merging on match_id would collapse or fail to
    # preserve visit-level predictions.
    candidate_cols = [
        "connectome_key", "runno", "graph_id", "Subject_ID", "subject_match",
        "regional_id", "match_id", "subject_id", "PTID", "ptid", "RID", "ID"
    ]
