...
def candidate_input_paths(cohort: str, feature_set: str) -> list[tuple[str, Path]]:
    paths = [
...
def read_first_available(cohort: str, feature_set: str) -> tuple[Optional[pd.DataFrame], str, str]:
    for source_type, path in candidate_input_paths(cohort, feature_set):
        if path.exists():
            try:
                return pd.read_csv(path, low_memory=False), source_type, str(path)
...
def metrics_for_one(cohort: str, feature_set: str, subset: str = "all_subjects") -> dict:
    df, source_type, source_path = read_first_available(cohort, feature_set)
...
def main() -> None:
    ...
    readme.write_text(
        ...
        "1. `<prefix>_<feature_set>_full_cohort_predictions.csv`\n"
        "2. `<prefix>_<feature_set>_metadata_all_with_predictions.csv`\n"
        "3. `validation_figures/subject_level_validation_input.csv` only as fallback\n\n"
