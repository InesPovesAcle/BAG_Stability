...
# -*- coding: utf-8 -*-
"""
Created on Wed Mar 25 15:06:21 2026
...
base_path_adni_dti =  os.path.join(
    WORK,"ines/data/harmonization/ADNI/connectomes/DWI/plain")
...
for folder_name in os.listdir(base_path_adni_dti):
    ...
    if os.path.isdir(folder_path):
        ...
        if "_connectomics" in folder_name:
            ...
            file_name = f"{connectome_id}_conn_plain.csv"
...
for file in os.listdir(base_path_adni_dti):
    if file.endswith("_conn_plain.csv"):
        file_path = os.path.join(base_path_adni_dti, file)
        df = pd.read_csv(file_path, header=None)
        subject_id = file.replace("_conn_plain.csv", "")
        connectomes[subject_id] = df

print(f"Total connectome matrices loaded: {len(connectomes)}")
print("Example keys:", list(connectomes.keys())[:5])
...
metadata_path_adni = os.path.join(
    WORK,"ines/data/harmonization/ADNI/ADNI_metadata.csv")
df_adni_metadata = pd.read_csv(metadata_path_adni)
...
shared_keys_adni = sorted(
    list(set(adni_connectomes_dti.keys()) & set(adni_connectomes_dti.keys()))
)
...
fa_path = os.path.join(
    WORK,"ines/data/Regional_stats/ADNI/ADNI_studywide_stats_for_fa.txt")
df_fa = pd.read_excel(fa_path, sep="\t")[1:]
...
vol_path = os.path.join(
    WORK,"ines/data/Regional_stats/ADNI/ADNI_studywide_stats_BrainPct.csv")
