...
# If the explicit connectome folders above are incomplete, auto-discover likely
# connectome folders/files under the cohort harmonization directory. This is
# especially useful for cohorts whose connectomes are not stored in the same
# structure as ADNI/HABS.
AUTO_DISCOVER_CONNECTOMES = True

COHORT_DIR_ALIASES = {
    "ADNI": ["ADNI"],
    "ADRC": ["ADRC"],
    "HABS": ["HABS"],
    "AD_DECODE": ["AD_DECODE", "ADDECODE", "AD-DECODE"],
}

CONNECTOME_FILENAME_HINTS = [
    "conn", "connectome", "connectivity", "matrix", "plain", "graph", "dwi"
]
...
def normalize_connectome_key(x: object) -> object:
    """
    Normalize metadata DWI keys and connectome filenames.

    Examples:
      ADNI:      R6703_y4_conn_plain.csv -> R6703_y4
      HABS:      H4369_y0_connectome.csv  -> H4369_y0
      AD-DECODE: S00775_conn_plain.csv    -> S00775
                 S00775_whitematter_conn_plain.csv -> S00775
    """
    if pd.isna(x):
        return pd.NA
    ...
    # If value is a path, operate on basename stem.
    s = Path(s).stem

    # Remove macOS hidden-resource prefix if present.
    s = re.sub(r"^\._", "", s)

    # AD-DECODE keys, e.g. S00775_conn_plain -> S00775.
    # Put this before generic suffix cleanup so metadata values containing Sxxxxx
    # also normalize to the same connectome key.
    m = re.search(r"(S\d+)", s, flags=re.I)
    if m:
        return m.group(1).upper()

    # ADNI keys, e.g. R6703_y4_conn_plain -> R6703_y4.
    ...
    # HABS keys, e.g. H4369_y0_connectome -> H4369_y0.
    ...
    # ADRC connectome keys, e.g. D0007_conn_plain -> D0007.
    # Metadata IDs such as ADRC0007 are handled in normalize_metadata_key_for_matching().
    m = re.search(r"(D\d+)", s, flags=re.I)
    if m:
        dnum = re.sub(r"^D", "", m.group(1), flags=re.I)
        try:
            return "D" + str(int(dnum)).zfill(4)
        except Exception:
            return m.group(1).upper()

    # Generic cleanup.
    low_suffixes = [
        "_whitematter_conn_plain",
        "_conn_plain",
        "_plain",
        "_connectome",
        "_connectivity",
...
def cohort_search_dirs(cohort: str) -> list[Path]:
    """Return existing cohort directories to search for connectomes."""
    dirs: list[Path] = []
    for alias in COHORT_DIR_ALIASES.get(cohort, [cohort]):
        for root in [BASE / alias, BASE / alias.replace("_", ""), BASE / alias.replace("-", "_")]:
            if root.exists() and root not in dirs:
                dirs.append(root)
    return dirs


def looks_like_connectome_file(path: Path) -> bool:
    """Heuristic for auto-discovered files: keep plausible connectome files only."""
    if path.suffix.lower() not in CONNECTOME_EXTENSIONS:
        return False
    name = path.name.lower()
    parent = str(path.parent).lower()
    return any(h in name or h in parent for h in CONNECTOME_FILENAME_HINTS)


def discover_connectome_files(cohort: str) -> list[Path]:
    """Auto-discover likely connectome files under cohort harmonization folders."""
    if not AUTO_DISCOVER_CONNECTOMES:
        return []
    files: list[Path] = []
    for root in cohort_search_dirs(cohort):
        for ext in CONNECTOME_EXTENSIONS:
            for f in root.rglob(f"*{ext}"):
                if looks_like_connectome_file(f):
                    files.append(f)
    return sorted(set(files))



def normalize_metadata_key_for_matching(x: object, cohort: str, colname: str = "") -> object:
    """Normalize metadata-side keys for connectome matching.

    Cohort-specific details:
      AD-DECODE:
        metadata DWI/MRI_Exam values are numeric IDs such as 2110, while
        connectome filenames are S02110_conn_plain.csv. Convert 2110 -> S02110.

      ADRC:
        connectome filenames are D0007_conn_plain.csv, while metadata may store
        PTID/subject IDs as ADRC0007. Convert ADRC0007 -> D0007.
    """
    base = normalize_connectome_key(x)
    if pd.isna(base):
        return pd.NA
    ...
    if cohort == "AD_DECODE":
        # Already canonical, e.g. S02110.
        m = re.fullmatch(r"S(\d+)", s, flags=re.I)
        if m:
            return "S" + str(int(m.group(1))).zfill(5)

        # Numeric metadata IDs, e.g. 2110 or 2110.0 -> S02110.
        # This is intended for DWI / MRI_Exam / MRI_Exam_fixed-like columns.
        if re.fullmatch(r"\d+(?:\.0)?", s):
            return "S" + str(int(float(s))).zfill(5)

        # Values containing Sxxxxx are already extracted by normalize_connectome_key.
        return s.upper()

    if cohort == "ADRC":
        # Already canonical connectome key, e.g. D0007.
        m = re.fullmatch(r"D(\d+)", s, flags=re.I)
        if m:
            return "D" + str(int(m.group(1))).zfill(4)

        # Metadata subject IDs, e.g. ADRC0007 -> D0007.
        m = re.search(r"ADRC\s*0*(\d+)", raw, flags=re.I)
        if m:
            return "D" + str(int(m.group(1))).zfill(4)

        # Some metadata may store numeric subject IDs only. Be conservative:
        # only convert numeric values in subject-like columns to D####.
        cname = normalize_name(colname)
        subject_like = any(k in cname for k in ["ptid", "subject", "match", "id"])
        if subject_like and re.fullmatch(r"\d+(?:\.0)?", s):
            return "D" + str(int(float(s))).zfill(4)

        return s.upper()

    return base

def extract_metadata_keys_from_multiple_columns(df: pd.DataFrame, cohort: str = "") -> pd.DataFrame:
    """Return a long dataframe of normalized metadata keys from all plausible key columns."""
    key_cols = [
        "DWI", "DWI_key", "DWI_connectome_key",
        "connectome_key", "connectome_full_key",
        "graph_key", "graph_id",
        "runno", "subject_visit_key", "dwi_key",
        "MRI_Exam", "MRI_Exam_fixed", "MRI_EXAM", "MRIEXAM",
        "ID", "id", "match_id", "Match_ID",
        "subject_id", "Subject", "PTID",
        "IDRNA", "ID_RNA", "ID_PCA", "IDPCA",
        "SubjID", "SUBJID", "participant_id",
    ]
    ...
    for c in key_cols:
        hit = first_col(df, [c])
        if hit is None or hit in seen_cols:
            continue
        seen_cols.append(hit)
        vals = df[hit].map(lambda v: normalize_metadata_key_for_matching(v, cohort, hit))
        rows.append(pd.DataFrame({
            "_row_index": df.index,
            "_metadata_key_col": hit,
            "_normalized_key": vals,
        }))

    if not rows:
        return pd.DataFrame(columns=["_row_index", "_metadata_key_col", "_normalized_key"])

    out = pd.concat(rows, ignore_index=True)
    out = out.dropna(subset=["_normalized_key"])
    out["_normalized_key"] = out["_normalized_key"].astype(str)
    return out


def collect_addecode_main_connectome_keys() -> tuple[set[str], dict[str, object], dict[str, str]]:
    """
    Collect the intended AD-DECODE main connectomes.

    Use only top-level files matching:
      Sxxxxx_conn_plain.csv

    Exclude:
      - macOS hidden resource fork files, e.g. ._S00775_conn_plain.csv
      - white matter variants, e.g. S00775_whitematter_conn_plain.csv
      - nested backup/connectome_act files
      - mean_FA, distances, sift, zip, etc.
    """
    roots = [
        BASE / "AD_DECODE" / "connectomes",
        BASE / "ADDECODE" / "connectomes",
    ...
    if root is None:
        return set(), {
            ...
            "n_connectome_keys": 0,
            "ad_decode_main_plain_connectomes_only": "yes",
        }, {}

    files = sorted(
        f for f in root.glob("S*_conn_plain.csv")
        if f.is_file()
        and not f.name.startswith("._")
        and "whitematter" not in f.name.lower()
    )

    key_to_file: dict[str, str] = {}
    for f in files:
        key = normalize_connectome_key(f.name)
        if pd.notna(key):
            key_to_file.setdefault(str(key), str(f))

    info = {
        "connectome_scan_status": "ok" if key_to_file else "no_files_found",
        "connectome_roots_used": str(root),
        "n_connectome_files": len(files),
        "n_connectome_keys": len(key_to_file),
        "n_auto_discovered_connectome_files": 0,
        "ad_decode_main_plain_connectomes_only": "yes",
    }
...
def collect_adrc_main_connectome_keys() -> tuple[set[str], dict[str, object], dict[str, str]]:
    """
    Collect the intended ADRC main connectomes when stored as D####_conn_plain.csv.

    Metadata often stores PTID as ADRC0007, while the connectome file is
    D0007_conn_plain.csv. This collector mirrors the AD-DECODE top-level plain
    connectome logic and excludes hidden / white-matter / nested backup files.

    If no top-level D*_conn_plain.csv files are found, the generic recursive
    collector will be used by collect_connectome_keys().
    """
    roots = [
        BASE / "ADRC" / "connectomes",
        BASE / "ADRC" / "connectomes" / "DWI" / "plain",
    ...
    files: list[Path] = []
    roots_used: list[Path] = []
    for root in roots:
        if not root.exists():
            continue
        roots_used.append(root)
        # Prefer top-level plain files in each candidate root.
        files.extend(
            f for f in root.glob("D*_conn_plain.csv")
            if f.is_file()
            and not f.name.startswith("._")
            and "whitematter" not in f.name.lower()
        )
    ...
    info = {
        "connectome_scan_status": "ok" if key_to_file else "no_top_level_D_plain_files_found",
        "connectome_roots_used": ";".join(str(p) for p in roots_used),
        "n_connectome_files": len(files),
        "n_connectome_keys": len(key_to_file),
        "n_auto_discovered_connectome_files": 0,
        "adrc_main_plain_connectomes_only": "yes" if key_to_file else "no",
    }

    return set(key_to_file), info, key_to_file

def collect_connectome_keys(cohort: str) -> tuple[set[str], dict[str, object], dict[str, str]]:
    """Collect connectome keys from explicit roots plus auto-discovered files."""
    # AD-DECODE needs special handling: the intended analysis files are only the
    # top-level Sxxxxx_conn_plain.csv files. Broad recursive scans pick up
    # white-matter, mean_FA, distances, backup, and macOS resource-fork files.
    if cohort == "AD_DECODE":
        return collect_addecode_main_connectome_keys()

    # ADRC can similarly have main connectomes named D####_conn_plain.csv while
    # metadata has PTID values such as ADRC0007. Prefer those top-level main
    # files when present; otherwise fall back to the generic scanner below.
    if cohort == "ADRC":
        keys, info, key_to_file = collect_adrc_main_connectome_keys()
        if keys:
            return keys, info, key_to_file
    ...
    discovered_files = discover_connectome_files(cohort)
    files = sorted(set(files) | set(discovered_files))

    # Exclude macOS resource forks everywhere.
    ...
    roots_used = sorted(set([str(p) for p in roots] + [str(f.parent) for f in discovered_files]))
    status = "ok" if key_to_file else ("no_files_found" if roots or cohort_search_dirs(cohort) else "no_root_found")
    info = {
        "connectome_scan_status": status,
        "connectome_roots_used": ";".join(roots_used),
        "n_connectome_files": len(files),
        "n_connectome_keys": len(key_to_file),
        "n_auto_discovered_connectome_files": len(discovered_files),
    }
    return set(key_to_file), info, key_to_file
...
def add_connectome_match_columns(df: pd.DataFrame, cohort: str) -> tuple[pd.DataFrame, dict[str, object]]:
    """Add clean connectome key/path columns and filter to rows backed by actual connectome files.

    This version checks ALL plausible metadata key columns against actual connectome
    filenames, not just the first detected key column. It writes which metadata
    column matched each row.
    """
    out = df.copy()
    ...
    # Preferred: actual filename scan against every plausible metadata key column.
    if keys:
        long_keys = extract_metadata_keys_from_multiple_columns(out, cohort)
        if not long_keys.empty:
            long_keys["_is_match"] = long_keys["_normalized_key"].isin(keys)
            matched = long_keys.loc[long_keys["_is_match"]].copy()
            if not matched.empty:
                # If a row matches via multiple columns, keep the first occurrence.
                matched = matched.drop_duplicates("_row_index", keep="first")
                match_key = pd.Series(pd.NA, index=out.index, dtype="object")
                match_col = pd.Series(pd.NA, index=out.index, dtype="object")
                match_key.loc[matched["_row_index"].values] = matched["_normalized_key"].values
                match_col.loc[matched["_row_index"].values] = matched["_metadata_key_col"].values

                mask = match_key.notna()
                out["CONNECTOME_KEY_USED_FOR_INTERSECTION"] = match_key
                out["CONNECTOME_METADATA_COLUMN_USED"] = match_col
                out["MATCHED_CONNECTOME_FILE"] = out["CONNECTOME_KEY_USED_FOR_INTERSECTION"].map(key_to_file)
                # Prefer the exact matched key as the clean key.
                out["CONNECTOME_KEY_CLEAN"] = out["CONNECTOME_KEY_USED_FOR_INTERSECTION"].combine_first(out["CONNECTOME_KEY_CLEAN"])

                if FILTER_TO_CONNECTOME_ROWS:
                    out_f = out.loc[mask].copy()
                    info.update({
                        "connectome_filter_applied": "yes",
                        "connectome_filter_method": "connectome_filename_scan_all_key_columns",
                        "metadata_rows_after_connectome_filter": len(out_f),
                        "n_matched_metadata_keys": int(mask.sum()),
                        "matched_metadata_key_columns": ";".join(sorted(set(matched["_metadata_key_col"].astype(str)))),
                    })
                    return out_f, info
    ...
    if FILTER_TO_CONNECTOME_ROWS and has_col is not None:
        ...
        out["CONNECTOME_METADATA_COLUMN_USED"] = key_col or ""
        out_f = out.loc[mask].copy()
        info.update({
            "connectome_filter_applied": "yes",
    ...
    if FILTER_TO_CONNECTOME_ROWS and path_col is not None:
        ...
        out["CONNECTOME_KEY_USED_FOR_INTERSECTION"] = out["CONNECTOME_KEY_CLEAN"].where(mask, pd.NA)
        out["CONNECTOME_METADATA_COLUMN_USED"] = path_col
        out_f = out.loc[mask].copy()
        info.update({
            "connectome_filter_applied": "yes",
...
def harmonize_ad_decode(df: pd.DataFrame) -> tuple[pd.DataFrame, str]:
    ...
    if risk_col is not None:
        ...
        is_normal = s.isin(["NORMAL", "CONTROL", "CONTROLS", "CN", "NC", "RISK0", "RISK 0", "RISK1", "RISK 1"])
...
def harmonize_adrc(df: pd.DataFrame) -> tuple[pd.DataFrame, str]:
    """
    Harmonize ADRC diagnostic status.

    ADRC commonly has three binary columns:
      NORMCOG  / NORMCOG_01 : 1 = cognitively normal
      IMPNOMCI / MCI_01     : 1 = impaired-not-MCI / MCI-like group
      DEMENTED / DEMENTIA_01: 1 = dementia/AD

    Important patch:
    Rows with NORMCOG=0 and DEMENTED=0 but missing IMPNOMCI should not remain
    fully Unknown. They are known not-normal and not-demented, so label them as
    Other/Impaired non-demented and set MCI_01=0.
    Rows with all status columns missing remain Unknown.
    """
    df = df.copy()
    normcog, mci, ad, label = init_status(df)
    sources = []

    # Include typo observed in some outputs: NORMCONG_01.
    norm_col = first_col(df, ["NORMCOG_01", "NORMCOG", "normcog", "NORMCONG_01"])
    ...
    if norm_col is not None:
        ...
        # If explicitly normal, other diagnostic indicators should be zero unless contradicted later.
        mci.loc[x.eq(1) & mci.isna()] = 0
        ad.loc[x.eq(1) & ad.isna()] = 0
    ...
    # Clean cross-field implications.
    mci.loc[normcog.eq(1) & mci.isna()] = 0
    ad.loc[normcog.eq(1) & ad.isna()] = 0

    normcog.loc[mci.eq(1) & normcog.isna()] = 0
    ad.loc[mci.eq(1) & ad.isna()] = 0

    normcog.loc[ad.eq(1) & normcog.isna()] = 0
    mci.loc[ad.eq(1) & mci.isna()] = 0

    # Known not-normal, not-demented, but no MCI flag: classify as other impaired/non-demented.
    known_non_normal_non_demented = normcog.eq(0) & ad.eq(0) & mci.isna()
    mci.loc[known_non_normal_non_demented] = 0

    # Label priority: AD > MCI > Normal > known other > Unknown.
    label.loc[normcog.eq(1)] = "Normal"
    label.loc[mci.eq(1)] = "MCI"
    label.loc[ad.eq(1)] = "AD"
    known_other = normcog.eq(0) & mci.eq(0) & ad.eq(0)
    label.loc[known_other] = "Other/Impaired non-demented"
...
def main() -> None:
    ...
    for cohort, candidates in METADATA_CANDIDATES.items():
        ...
        # Save a small connectome-matching audit for each cohort.
        audit_cols = [
            c for c in [
                "CONNECTOME_KEY_CLEAN", "CONNECTOME_KEY_USED_FOR_INTERSECTION",
                "CONNECTOME_METADATA_COLUMN_USED", "MATCHED_CONNECTOME_FILE",
                "DWI", "DWI_key", "connectome_key", "connectome_full_key", "graph_id",
                "MRI_Exam", "MRI_Exam_fixed", "ID", "match_id", "subject_id", "runno",
                "risk_for_ad", "DX_Label_harmonized", "NORMCOG_01", "MCI_01", "AD_01",
            ] if c in out.columns
        ]
        connectome_audit_path = OUTDIR / f"{cohort}_connectome_match_audit.csv"
        out[audit_cols].to_csv(connectome_audit_path, index=False)
        ...
        row: dict[str, object] = {
            ...
            "connectome_match_audit_file": str(connectome_audit_path),
        }
        ...
        print(f"       connectome filter: {connectome_info.get('connectome_filter_method')} | files={connectome_info.get('n_connectome_files')} | matched={connectome_info.get('n_matched_metadata_keys')}")
