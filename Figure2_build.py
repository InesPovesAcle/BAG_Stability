...
FIGURE_CONFIGS = [
    ...
    },
    {
        ...
        "title": "Bias-corrected performance diagnostics across cohorts and feature sets",
        "basename": "SupplementaryFigure2_OOF_BIAS_CORRECTED_performance_bootstrap_CI_2x2",
    },
]
...
def get_axis_limits(metric, data):
    ...
    finite_vals = pd.concat([vals, lows, highs], ignore_index=True).replace(
        [np.inf, -np.inf],
        np.nan,
    ).dropna()
...
def make_2x2_figure(df, evaluation, title, basename):
    ...
    out_png = os.path.join(BASE_DIR, f"{basename}.png")
    out_pdf = os.path.join(BASE_DIR, f"{basename}.pdf")
    out_csv = os.path.join(BASE_DIR, f"{basename}_source_data.csv")
